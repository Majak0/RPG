...
export default {
  ...
  data: () => {
    return {
      currentTown: town, // la ville courante
  ...
  methods: {
    selectStreet(id) {
      ...
      */
      let index = this.currentTown.streets.findIndex(street => street.name === id)

      // Si l'index existe
      if (index !== -1){
        this.currentStreet = this.currentTown.streets[index]
    ...
    selectShop(id) {
      ...
      */
      let index = this.currentStreet.shops.findIndex(shop => shop.name === id)

      // Si l'index existe
      if (index !== -1){
        this.currentShop = this.currentStreet.shops[index]
      }
    },
    assign() {
      /* EXO 4.6
